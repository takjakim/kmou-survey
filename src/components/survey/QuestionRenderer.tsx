...
export function ScaleMatrixRenderer({ questions, values, onChange, validationErrors }: ScaleMatrixProps) {
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="text-left text-xs font-medium text-gray-500 px-4 py-2 w-auto">항목</th>
              {scalePoints.map((point) => (
                <th
                  key={point}
                ...
                >
                  ...
                  {SCALE_LABELS[point] && (
                    <div className="text-gray-400 font-normal leading-tight mt-0.5">
                      {SCALE_LABELS[point][0] && <div>{SCALE_LABELS[point][0]}</div>}
                      <div>{SCALE_LABELS[point][1]}</div>
                    </div>
                  )}
                </th>
              ))}
                    ...
                    return (
                      <td key={point} className="px-2 py-2.5 text-center">
                        <button
                          ...
                          aria-label={`${question.title} - ${SCALE_LABELS[point]?.join('') ?? point}점`}
                  ...
                  {hasNA && (
                    <td className="px-2 py-2.5 text-center">
                      <button
                        ...
                        aria-label={`${question.title} - 해당없음`}
                ...
                {scalePoints.map((point) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => onChange(question.id, point)}
                      aria-label={`${SCALE_LABELS[point]?.join('') ?? point}점`}
